		...
		public override MonitorStates GetState()
		{
            ...
            try
            {
                ...
                foreach (DatabaseEntry dbEntry in databases)
                {
                    ...
                    {
                        warningCount++;
                        plainTextDetails.AppendLine(string.Format("Database {0} - Size {1}MB - Warning (trigger {2}MB", dbEntry.Name, size, dbEntry.ErrorSizeMB));
                        htmlTextTextDetails.AppendLine(string.Format("<li>Database {0} - Size {1}MB - <b>Warning</b> (trigger {2}MB)</li>", dbEntry.Name, size, dbEntry.ErrorSizeMB));
                    }
                    else
                    {
                        okCount++;
