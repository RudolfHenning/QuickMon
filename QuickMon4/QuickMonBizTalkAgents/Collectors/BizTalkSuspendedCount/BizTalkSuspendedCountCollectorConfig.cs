...
namespace QuickMon.Collectors
{
    ...
    public class BizTalkSuspendedCountCollectorConfigEntry : BizTalkGroupBase, ICollectorConfigEntry
    {
        ...
        public List<SuspendedInstance> GetTopXSuspendedInstanced(int topCount = 0)
        {
            if (topCount == 0)
                topCount = ShowLastXDetails;
            List<SuspendedInstance> list = new List<SuspendedInstance>();
            if (topCount > 0)
            {
                foreach (string connString in GetMsgBoxConnStrs())
                {
                    foreach (string suspendedQueue in GetHostInstanceSuspendedQs(connString))
                    {
                        ...
                        {
                            list.AddRange(GetHostSuspendedInstancesLastX(connString, suspendedQueue, topCount));
                        }
                    }
                }
            }
        ...
        private IEnumerable<SuspendedInstance> GetHostSuspendedInstancesLastX(string connString, string hostInstanceSuspendedQ, int topCount)
        {
