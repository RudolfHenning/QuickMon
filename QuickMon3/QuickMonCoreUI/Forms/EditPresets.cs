...
namespace QuickMon.Forms
{
    public partial class EditPresets : Form
    {
        ...
        private void lvwPresets_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            exportToolStripButton.Enabled = lvwPresets.SelectedItems.Count > 0;
            deleteTemplateToolStripMenuItem.Enabled = lvwPresets.SelectedItems.Count > 0;
            exportSelectedTemplatesToolStripMenuItem.Enabled = lvwPresets.SelectedItems.Count > 0;
        ...
        private void exportToolStripButton_Click(object sender, EventArgs e)
        {
            try
            {
                if (lvwPresets.SelectedItems.Count > 0)
                {
                    if (saveTemplateFileDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        List<AgentPresetConfig> list = new List<AgentPresetConfig>();
                        foreach (ListViewItem lvi in lvwPresets.SelectedItems)
                        {
                        ...
                        AgentPresetConfig.SavePresetsToFile(saveTemplateFileDialog.FileName, list);
                    }
                }
            }
            catch (Exception ex)
